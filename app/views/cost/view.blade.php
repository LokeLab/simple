...
{{ Form::open(array('url' => 'cost/'.$v->id.'/reject', 'method' => 'POST', 'files' => true)) }}
    
<div class="row">
	<div class="col-lg-3">
		<div class="form-group">
		@if ($v->active=0 || Auth::user()->role == 1)
		<a href="{{ url('cost/'.$v->id.'/edit') }}" class="btn blue">{{Lang::get('generic.edit');}}</a>
		@endif
		@if ( Auth::user()->role == 1)
		<a href="{{ url('cost/'.$v->id.'/approve') }}" class="btn green">{{Lang::get('generic.approve');}}</a>
		@endif	
